...
const Setting2 = () => {
  ...
  // === GHÉP ẢNH THEO ĐÚNG LOGIC SELPHOTO (CẢ CHỤP & UPLOAD) ===
 // === GHÉP ẢNH THEO ĐÚNG LOGIC SELPHOTO & FRAME (ĐÃ SỬA LỖI CUT=3, FIT, Ô ĐEN) ===
const generateCompositeImage = async (photos) => {
  ...
  // === CÁC LAYOUT ĐÃ ĐƯỢC ĐỒNG BỘ HOÀN TOÀN VỚI SELPHOTO & FRAME ===
  if (cutValue === '42') {
    compositeWidth = 600; compositeHeight = 900;
    const paddingLeft = 5, paddingTop = 30, paddingRight = 5, paddingBottom = 120, gap = 1;
    ...
    const paddingLeft = 12, paddingTop = 25, paddingRight = 12, paddingBottom = 90, gap = 10;
    ...
    const paddingLeft = 25, paddingTop = 40, paddingRight = 25, paddingBottom = 40, gap = 11;
    ...
    // CHỈ DÙNG 3 VỊ TRÍ → KHÔNG CÓ Ô ĐEN THỨ 4
  }
  else if (cutValue === '6') {
    compositeWidth = 600; compositeHeight = 900;
    const paddingLeft = 10, paddingRight = 20, paddingTop = 24, paddingBottom = 120, gap = 1;
    imageWidth = (compositeWidth - paddingLeft - paddingRight - gap) / 2;
  ...
  const handleSampleUpload = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const photos = Array(count).fill(photo);
      generateCompositeImage(photos);
    };
